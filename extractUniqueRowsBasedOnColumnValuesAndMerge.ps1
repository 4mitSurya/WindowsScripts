...
$outputFileDir = "$inputFileDir\extractUniqueRows_output"
...
if (Test-Path $outputFileDir) {
    ...
    New-Item -ItemType Directory -Force -Path "$inputFileDir\extractUniqueRows_output"
...
Foreach-Object {
	...
	$uniqueFromInputs=(Import-Csv $_.FullName | Sort $columns -Unique | Select *)
	$uniqueFromInputs | Export-Csv -Path $outputFile -Append -NoTypeInformation
	
	$uniqueFromOutput=(Import-Csv $outputFile | Sort $columns -Unique | Select *)
